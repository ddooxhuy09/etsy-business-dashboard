...
Uses PostgreSQL via shared.db.run_query.
"""
import math
import pandas as pd
...
from features.report.pdf import create_pdf_report
...
def bank_accounts(offset: int = Query(0, ge=0), limit: int = Query(100, ge=1, le=50000)):
    sql = """WITH bank_account_stats AS (
            ...
            SELECT
                ...
                ROUND(bas.total_credit::numeric, 4) as "Total Credit (USD)",
                ROUND(bas.total_debit::numeric, 4) as "Total Debit (USD)",
                ROUND(bas.current_balance::numeric, 4) as "Current Balance (USD)",
