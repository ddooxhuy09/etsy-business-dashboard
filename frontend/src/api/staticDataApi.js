...
export function updateBankTransaction(key, updates) {
  return api.put(`/api/static/bank-transactions/${key}`, updates).then((r) => r.data);
}
...
// PL Account Config API
export function fetchPlAccounts() {
  return api.get('/api/pl-accounts').then((r) => r.data);
}

export function createPlAccount(data) {
  return api.post('/api/pl-accounts', data).then((r) => r.data);
}

export function updatePlAccount(accountNumber, data) {
  return api.put(`/api/pl-accounts/${accountNumber}`, data).then((r) => r.data);
}

export function deletePlAccount(accountNumber) {
  return api.delete(`/api/pl-accounts/${accountNumber}`).then((r) => r.data);
}
