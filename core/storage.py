...
def read_periods_data() -> dict:
    """
    Read full periods.json from bucket root.
    Format: {"periods": ["2025-01", ...], "metadata": {"2025-01": {"etl_done_at": "...", "file_count": 3}}}

    Returns:
        dict with "periods" list and "metadata" dict (empty dicts if not found)
    """
    try:
        data = read_json_from_storage("periods.json")
        ...
        import traceback
        print(f"Error reading periods.json: {e}")
        print(traceback.format_exc())
    return {"periods": [], "metadata": {}}


def write_periods_data(periods: list, metadata: dict) -> bool:
    """
    Write full periods.json to bucket root (periods list + per-period metadata cache).

    Args:
        periods: Sorted list of period strings
        metadata: Dict mapping period -> {"etl_done_at": ..., "file_count": ...}

    Returns:
        True if successful, False otherwise
    """
    try:
        data = {"periods": sorted(periods), "metadata": metadata}
        result = write_json_to_storage("periods.json", data)
        if not result:
            print(f"Failed to write periods.json")
        return result
    except Exception as e:
        import traceback
        print(f"Exception writing periods.json: {e}")
        print(traceback.format_exc())
        return False
...
def list_all_periods() -> list:
    ...
    """
    try:
        periods_data = read_periods_data()
        if periods_data["periods"]:
            return periods_data["periods"]
...
def save_periods_list(periods: list) -> bool:
    """
    Save list of periods to periods.json in bucket root (preserves existing metadata).
    ...
    """
    try:
        existing = read_periods_data()
        result = write_periods_data(periods, existing.get("metadata", {}))
        if not result:
