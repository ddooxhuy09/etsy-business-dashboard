...
def query_products_optimized() -> List[Dict[str, Any]]:
    ...
    -- Pre-aggregate COGS by product only (sum ALL variants) so the product-level row
    -- reflects the total COGS regardless of which variant row the frontend picks first.
    cogs_agg AS (
        SELECT
