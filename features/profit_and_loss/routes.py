...
from features.profit_and_loss.summary_table import get_profit_loss_summary_table
from features.profit_and_loss.formula_config import (
...
def summary_table(
    ...
    use_default_formula: If True (default) and selected_items is not provided,
                        Net Profit will be calculated using the default formula from formula_config.py.
    ...
    try:
        logger.info(
            ...
            start_date, end_date, view_mode, selected_items, use_default_formula,
        )
        selected_list = None
        if selected_items:
...
def clean_bank_by_pl(
    ...
    logger.warning(
        ...
        start_date, end_date, accounts, account_number,
    )
